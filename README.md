...
一個功能強大的 Python PPT 生成工具,支持從 JSON 格式讀取內容,套用 .pptx 模板,並輸出包含文字、圖片、表格、圖表的專業簡報。

## 功能特色

- **JSON 格式輸入**: 使用結構化的 JSON 格式定義簡報內容
- **16:9 比例**: 自動生成標準 16:9 寬屏比例的簡報 (13.333 x 7.5 英寸)
- **模板支持**: 可以套用現有的 .pptx 模板文件
...
- **template**: (可選) 模板文件路徑,如果不指定或文件不存在,將創建新的簡報
- **output**: (必需) 輸出文件路徑
- **slides**: (必需) 投影片數組,每個元素定義一張投影片

**注意**: 無論是否使用模板，生成的簡報都會自動設置為 16:9 比例 (13.333 x 7.5 英寸)。
...
您可以創建自己的 .pptx 模板文件,工具會使用模板中定義的樣式:

1. 在 PowerPoint 中創建一個模板文件
2. 設置主題、顏色方案、字體等
3. 保存為 .pptx 文件
4. 在 JSON 配置中指定模板路徑
...
```json
{
  "template": "my_custom_template.pptx",
  "output": "my_presentation.pptx",
  "slides": [...]
}
```

**重要**: 無論模板原始比例是什麼,工具都會自動將其調整為 16:9 比例。
...
2. **模板兼容性**: 不同的模板可能有不同數量的佈局,工具會自動選擇最合適的佈局
3. **數據格式**: 表格和圖表的數據必須格式正確,否則可能導致生成失敗
...
A: 請確保模板文件路徑正確,且文件確實存在。工具會在控制台輸出相關信息。
